...
<div class="statistic-container">
	...
	<button v-show="successCnt === complextCnt && passNext && isStarted" @click="nextNote" class="next control-item ">Hear Next</button>
...
function keyNoteClick(absIndex: number) {
	...
	if (isStarted.value) {
		...
		// if (absIndex === currentNoteInfo.absNoteIndex) {
			...
			passNext.value = true
			successCnt++;
			if (successCnt === complextCnt.value && autoNext.value) {
				successCnt = 0;
				nextNote()
			}
		} else {
